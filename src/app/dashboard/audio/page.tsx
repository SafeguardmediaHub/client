...
const AudioForensics: FC<DashboardProps> = ({
    ...
    const [voiceActivityImage, setVoiceActivityImage] = useState<string | null>(null);
        ...
        <section className="flex flex-1 flex-col gap-4 py-4 px-8">
            <header className="flex-col items-start gap-1 flex">
                ...
                <p className="text-sm text-muted-foreground">
                    Advanced audio forensic analysis for detecting manipulation and verifying authenticity
                </p>
            </header>
            ...
            {!analysisResult ? (
                ...
                <Card className="p-4 md:p-6 w-full max-w-5xl mx-auto">
                    <CardContent className="p-0 space-y-6">
                        ...
                        <p className="text-lg font-medium">Audio Forensic Analysis Results</p>
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                            ...
                            <div className="space-y-4">
                                <div className="border rounded-lg overflow-hidden bg-muted">
                                    {spectrogramImage ? (
                                        <img src={spectrogramImage} alt="Spectrogram" className="w-full h-auto max-h-[400px] object-contain" />
                                    ) : (
                            ...
                            <div className={`p-6 rounded-lg border ${getRiskBgColor(analysisResult.user_friendly_summary.authenticity_score)} h-fit`}>
                                <h3 className="text-lg font-semibold mb-2">Overall Authenticity Score</h3>
                                    ...
                                    <div className="relative w-40 h-40">
                                        <svg className="w-full h-full transform -rotate-90" viewBox="0 0 100 100">
                                            ...
                                            <circle
                                                ...
                                                stroke={analysisResult.user_friendly_summary.authenticity_score >= 70 ? "#22c55e" : analysisResult.user_friendly_summary.authenticity_score >= 40 ? "#eab308" : "#ef4444"}
                                                strokeWidth="8" strokeLinecap="round"
                                                strokeDasharray={`${analysisResult.user_friendly_summary.authenticity_score * 2.51} 251`}
                                            />
                                        </svg>
                                        <div className="absolute inset-0 flex flex-col items-center justify-center">
                                            <span className={`text-3xl font-bold ${analysisResult.user_friendly_summary.authenticity_score >= 70 ? 'text-green-500' : analysisResult.user_friendly_summary.authenticity_score >= 40 ? 'text-yellow-500' : 'text-red-500'}`}>
                                                {analysisResult.user_friendly_summary.authenticity_score}%
                                            </span>
                                        </div>
                                    </div>

                                    <p className="text-lg font-semibold mt-4">{analysisResult.user_friendly_summary.status}</p>
                        ...
                        <div className="border rounded-lg p-6 space-y-6">
                                ...
                                <div className="flex items-center justify-between mb-2">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <span className="px-2 py-1 rounded text-xs font-medium bg-blue-100 text-blue-800">
                                            {analysisResult.user_friendly_summary.speaker_count} Speaker(s)
                                        </span>
                                    </div>
                                </div>
                                <p className="text-sm text-muted-foreground mb-1">
                                    <strong>What this means:</strong> Identifies who is speaking and when in the audio
                                </p>
                                <p className="text-sm text-muted-foreground mb-2">
                                    {analysisResult.user_friendly_summary.voice_activity_summary}
                                </p>
                                <p className="text-xs text-muted-foreground">
                                    Transcription: {analysisResult.user_friendly_summary.transcription}
                                </p>
                            </div>
                            ...
                            <div className="border-b pb-4">
                                ...
                                <p className="text-sm text-muted-foreground mb-1">
                                    <strong>What this means:</strong> Examines sound frequencies to detect unnatural patterns
                                </p>
                                <p className="text-sm text-muted-foreground">
                                    {analysisResult.user_friendly_summary.spectral_analysis_summary}
                                </p>
                            </div>
                            ...
                            <div>
                                <div className="flex items-center justify-between mb-2">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <span className={`px-2 py-1 rounded text-xs font-medium ${getStatusColor(analysisResult.user_friendly_summary.tampering_probability)}`}>
                                            {analysisResult.user_friendly_summary.tampering_probability}
                                        </span>
                                    </div>
                                </div>
                                <p className="text-sm text-muted-foreground mb-1">
                                    <strong>What this means:</strong> Looks for cuts, splices, or edited sections in the audio
                                </p>
                                <p className="text-sm text-muted-foreground">
                        ...
                        <div className="border-t pt-4 grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                            <div>
                                ...
                                <p className="font-medium">{analysisResult.user_friendly_summary.audio_info.format}</p>
                            </div>
                            <div>
                                <p className="text-muted-foreground">Duration:</p>
                                <p className="font-medium">{analysisResult.user_friendly_summary.audio_info.duration}</p>
                            </div>
                            <div>
                                <p className="text-muted-foreground">Sample Rate:</p>
                                <p className="font-medium">{analysisResult.user_friendly_summary.audio_info.sample_rate}</p>
                            </div>
                            <div>
                                <p className="text-muted-foreground">Bitrate:</p>
                                <p className="font-medium">{analysisResult.user_friendly_summary.audio_info.bitrate}</p>
                            </div>
                        </div>
                    </CardContent>
                </Card>
