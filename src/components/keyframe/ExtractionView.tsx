...
export function ExtractionView({
  ...
  const uploadFrameToLibrary = useCallback(
    async (frame: KeyframeFrame) => {
      ...
      const fileName = frame.fileName ?? `keyframe_${frame.index + 1}.jpg`;
  ...
  }, [frames, isBulkUploading, uploadFrameToLibrary, uploadedFrames]);
        ...
        <CardContent className="p-0 space-y-4">
          <div className="space-y-1">
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <span
                className={`text-xs font-medium px-2 py-0.5 rounded-full border ${statusClasses(effectiveStatus)}`}
              >
                {statusLabel[effectiveStatus]}
              </span>
            </div>
            <p className="text-sm text-muted-foreground">
              ...
              {extraction.videoMetadata?.durationSeconds
                ? ` • Video length ${formatTimestamp(extraction.videoMetadata.durationSeconds)}`
                : ""}
            </p>
          </div>
          ...
          {showShortVideoBanner && (
            <div className="flex items-start gap-3 p-4 bg-amber-50 border border-amber-200 rounded-lg">
              ...
              <p className="text-sm text-amber-900">
                You requested {requested} frames, but this video only supports
                up to {achievable} distinct frames. Showing all {achievable}.
              </p>
            </div>
          )}
          ...
          <div className="flex items-start gap-2 text-xs text-muted-foreground">
            ...
            <p>
              Keyframes are kept for 30 days. Download or re-use them before
              then.
            </p>
          </div>
        ...
        <Card className="p-4 md:p-6 w-full">
          <CardContent className="p-0 space-y-4">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
              <div>
                <h3 className="text-lg font-semibold">Extracted Keyframes</h3>
                <p className="text-sm text-muted-foreground">
                  ...
                  <div key={key} className="space-y-2">
                    <div className="relative rounded-lg overflow-hidden border bg-muted aspect-video">
                      {frame.signedUrl ? (
                        ...
                        <img
                          src={frame.signedUrl}
                          alt={frame.fileName ?? `Keyframe ${frame.index + 1}`}
